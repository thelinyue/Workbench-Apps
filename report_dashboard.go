...
<main class="dashboard">
    ...
    <section class="diagnostic-banner {{.DiagnosticClass}} mb-4" id="diagnosticSummary">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3"><div><div class="small fw-bold text-uppercase mb-1">快速诊断结论</div><div class="fw-semibold">{{.DiagnosticSummary}}</div></div><button class="btn btn-sm btn-outline-dark" type="button" data-detail-target="{{.DiagnosticTarget}}">查看详情</button></div>
    </section>
    ...
    <section id="resultsList">
        ...
        <article class="result-card mb-3" data-category="{{$result.Category}}" data-search="{{$result.File}} {{$result.Category}}">
            ...
            <div id="fileCollapse{{$fileIndex}}" class="collapse"><div class="issue-list">
                {{range $group := $result.GroupedIssues}}
                <details class="issue-group" data-keyword="{{$group.Keyword}}" data-severity="{{$group.Severity}}">
