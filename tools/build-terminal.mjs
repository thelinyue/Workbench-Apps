...
const root=resolve(dirname(fileURLToPath(import.meta.url)),'..'), appRoot=join(root,'apps','terminal'), dist=join(appRoot,'dist'), manifest=JSON.parse(await readFile(join(appRoot,'manifest.json'),'utf8'));
...
for (const dependency of ['ssh2', 'asn1', 'bcrypt-pbkdf', 'tweetnacl', 'safer-buffer']) await cp(join(root, 'node_modules', dependency), join(dist, 'node_modules', dependency), { recursive: true });
const zip=join(dist,`terminal-v${manifest.version}.zip`);await createDeterministicZip(dist,zip);const bytes=await readFile(zip);const release={appId:manifest.id,version:manifest.version,hostApiVersion:manifest.hostApiVersion,minWorkbenchVersion:manifest.minWorkbenchVersion,url:`https://github.com/thelinyue/Workbench-Apps/releases/download/terminal-v${manifest.version}/terminal-v${manifest.version}.zip`,size:bytes.byteLength,sha256:createHash('sha256').update(bytes).digest('hex')};if(process.env.HEPHAESTUS_APP_SIGNING_PRIVATE_KEY&&process.env.HEPHAESTUS_APP_SIGNING_KEY_ID)release.signature={keyId:process.env.HEPHAESTUS_APP_SIGNING_KEY_ID,signature:sign(null,bytes,createPrivateKey(process.env.HEPHAESTUS_APP_SIGNING_PRIVATE_KEY)).toString('base64')};await writeFile(join(dist,'release.json'),JSON.stringify(release,null,2));
