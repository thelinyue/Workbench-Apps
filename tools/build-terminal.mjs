...
const zip=join(dist,getReleaseAssetName(manifest.id,manifest.version));await createDeterministicZip(dist,zip);const bytes=await readFile(zip);const release={appId:manifest.id,version:manifest.version,hostApiVersion:manifest.hostApiVersion,minWorkbenchVersion:manifest.minWorkbenchVersion,url:getReleaseUrl(manifest.id,manifest.version),size:bytes.byteLength,sha256:createHash('sha256').update(bytes).digest('hex')};if(process.env.HEPHAESTUS_APP_SIGNING_PRIVATE_KEY&&process.env.HEPHAESTUS_APP_SIGNING_KEY_ID)release.signature={keyId:process.env.HEPHAESTUS_APP_SIGNING_KEY_ID,signature:sign(null,bytes,createPrivateKey(process.env.HEPHAESTUS_APP_SIGNING_PRIVATE_KEY)).toString('base64')};await writeFile(join(dist,'release.json'),JSON.stringify(release,null,2));
