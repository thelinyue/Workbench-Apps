...
describe('分析中心 backend Worker', () => {
  it('使用独立数据库并通过 Host RPC 保存带扫描间隔的设置', async () => {
    ...
    try {
      ...
      await expect(backend.invoke('settings.get', null)).resolves.toEqual({ directory: undefined, enabled: false, scanIntervalMinutes: 5 });
      await backend.invoke('settings.save', { directory: dataDirectory, enabled: true, scanIntervalMinutes: 1 });
      await expect(backend.invoke('settings.get', null)).resolves.toEqual({ directory: dataDirectory, enabled: true, scanIntervalMinutes: 1 });
      await expect(backend.invoke('settings.save', { directory: dataDirectory, enabled: true, scanIntervalMinutes: 0 })).rejects.toThrow('自动扫描间隔至少为 1 分钟');
    } finally {
      await backend.close();
    }
