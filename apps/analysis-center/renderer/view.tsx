import { ChevronLeft, CircleAlert, CircleCheck, Clock3, Copy, ExternalLink, FilePlus2, FolderOpen, LoaderCircle, MoreHorizontal, PanelRightOpen, Play, Save, ScanSearch, Settings, ShieldAlert, Upload, X } from 'lucide-react';
...
import { formatFileSize, getAnalysisStageItems, getNotificationActivation, getPackageTone, getWorkspaceGroups, type AnalysisTaskStage } from './workspace-presentation';
...
export function AnalysisCenterApp() {
  ...
  const [monitor, setMonitor] = useState<MonitorSettings>({ enabled: false, autoAnalyzeEnabled: true, scanIntervalMinutes: 1 });
  ...
  const [settingsDraft, setSettingsDraft] = useState<MonitorSettings>({ enabled: false, autoAnalyzeEnabled: true, scanIntervalMinutes: 1 });
