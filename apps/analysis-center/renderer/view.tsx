...
export function AnalysisCenterApp() {
  ...
  const loadRules = () => host.invoke('rules.getActive');
  const analyze = (packageId: string, scope: 'comprehensive' | 'storage' = 'comprehensive') => void run(async () => { const rules = await loadRules(); await host.invoke('analysis.start', { packageId, scope, rules }); }, '分析任务已创建。');
  const analyzeAll = () => void run(async () => { const rules = await loadRules(); await host.invoke('analysis.start-all-pending', { rules }); }, '已创建批量分析任务。');
